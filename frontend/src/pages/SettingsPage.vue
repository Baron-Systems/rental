    ...
    <div class="p-6 lg:p-8" dir="rtl">
      ...
      <div v-else class="animate-fade-in">
        ...
        <div v-if="activeTab === 'general'">
          <div v-if="saved" class="mb-4 text-sm text-emerald-600 font-medium">تم الحفظ بنجاح!</div>
          <div v-else-if="editMode" class="mb-4 text-sm text-gold-700 font-medium">وضع التعديل</div>

          <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <!-- هوية المؤجر -->
            <Card padding="md" title="هوية المؤجر">
              <div class="grid grid-cols-1 gap-4">
                <div class="space-y-1">
                  <template v-if="editMode">
                    <label class="text-xs font-medium text-navy-500">نوع المؤجر</label>
                    <select v-model="form.landlord_type" class="input-premium" @change="onLessorTypeChange">
                      <option value="person">شخص</option>
                      <option value="company">شركة / مؤسسة</option>
                    </select>
                  </template>
                  <template v-else>
                    <p class="text-xs font-medium text-navy-400">نوع المؤجر</p>
                    <p class="text-sm font-semibold text-navy-800">{{ form.landlord_type === 'company' ? 'شركة / مؤسسة' : 'شخص' }}</p>
                  </template>
                </div>
                <div class="space-y-1">
                  <template v-if="editMode">
                    ...
                    <input v-model="form.landlord_name" type="text" :placeholder="form.landlord_type === 'company' ? 'اسم الشركة' : 'الاسم الكامل'" class="input-premium" />
                  </template>
                  <template v-else>
                    <p class="text-xs font-medium text-navy-400">{{ form.landlord_type === 'company' ? 'اسم الشركة / المؤسسة' : 'الاسم الكامل' }}</p>
                    <p class="text-sm font-semibold text-navy-800">{{ form.landlord_name || 'غير محدد' }}</p>
                  </template>
                </div>
                <div class="space-y-1">
                  <template v-if="editMode">
                    ...
                    <input v-model="form.landlord_id" type="text" :placeholder="form.landlord_type === 'company' ? 'رقم السجل' : 'رقم الهوية'" class="input-premium" />
                  </template>
                  <template v-else>
                    <p class="text-xs font-medium text-navy-400">{{ form.landlord_type === 'company' ? 'رقم التسجيل / السجل التجاري' : 'رقم الهوية' }}</p>
                    <p class="text-sm font-semibold text-navy-800">{{ form.landlord_id || 'غير محدد' }}</p>
                  </template>
                </div>
              </div>
            </Card>
            ...
            <!-- بيانات التواصل -->
            <Card padding="md" title="بيانات التواصل">
              <div class="grid grid-cols-1 gap-4">
                <div class="space-y-1">
                  <template v-if="editMode">
                    <label class="text-xs font-medium text-navy-500">الهاتف</label>
                    <input v-model="form.landlord_phone" type="text" dir="ltr" placeholder="رقم الهاتف" class="input-premium" />
                  </template>
                  <template v-else>
                    <p class="text-xs font-medium text-navy-400">الهاتف</p>
                    <p class="text-sm font-semibold text-navy-800" dir="ltr">{{ form.landlord_phone || 'غير محدد' }}</p>
                  </template>
                </div>
                <div class="space-y-1">
                  <template v-if="editMode">
                    <label class="text-xs font-medium text-navy-500">العنوان</label>
                    <input v-model="form.landlord_address" type="text" placeholder="العنوان الكامل" class="input-premium" />
                  </template>
                  <template v-else>
                    <p class="text-xs font-medium text-navy-400">العنوان</p>
                    <p class="text-sm font-semibold text-navy-800">{{ form.landlord_address || 'غير محدد' }}</p>
                  </template>
                </div>
              </div>
            </Card>
            ...
            <!-- بيانات المفوض -->
            <Card v-if="form.landlord_type === 'company'" padding="md" title="بيانات المفوض" class="md:col-span-2">
              <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div class="space-y-1">
                  <template v-if="editMode">
                    <label class="text-xs font-medium text-navy-500">اسم المفوض <span class="text-red-500">*</span></label>
                    <input v-model="form.landlord_representative_name" type="text" placeholder="اسم المفوض" class="input-premium" />
                  </template>
                  <template v-else>
                    <p class="text-xs font-medium text-navy-400">اسم المفوض</p>
                    <p class="text-sm font-semibold text-navy-800">{{ form.landlord_representative_name || 'غير محدد' }}</p>
                  </template>
                </div>
                <div class="space-y-1">
                  <template v-if="editMode">
                    <label class="text-xs font-medium text-navy-500">صفة المفوض <span class="text-red-500">*</span></label>
                    <input v-model="form.landlord_representative_title" type="text" placeholder="مثلاً: مدير عام" class="input-premium" />
                  </template>
                  <template v-else>
                    <p class="text-xs font-medium text-navy-400">صفة المفوض</p>
                    <p class="text-sm font-semibold text-navy-800">{{ form.landlord_representative_title || 'غير محدد' }}</p>
                  </template>
                </div>
                <div class="space-y-1">
                  <template v-if="editMode">
                    <label class="text-xs font-medium text-navy-500">رقم هوية المفوض <span class="text-red-500">*</span></label>
                    <input v-model="form.landlord_representative_id" type="text" placeholder="رقم هوية المفوض" class="input-premium" />
                  </template>
                  <template v-else>
                    <p class="text-xs font-medium text-navy-400">رقم هوية المفوض</p>
                    <p class="text-sm font-semibold text-navy-800">{{ form.landlord_representative_id || 'غير محدد' }}</p>
                  </template>
                </div>
              </div>
            </Card>
            ...
            <!-- هوية الطباعة -->
            <Card padding="md" title="هوية الطباعة">
              <div class="space-y-1">
                <template v-if="editMode">
                ...
                <template v-else>
                  <p class="text-xs font-medium text-navy-400">شعار المؤجر</p>
            ...
            <!-- الإعدادات المالية -->
            <Card padding="md" title="الإعدادات المالية">
              <div class="grid grid-cols-1 gap-4">
                <div class="space-y-1">
                  <template v-if="editMode">
                    <label class="text-xs font-medium text-navy-500">العملة الأساسية</label>
                    <select v-if="!currencyLocked" v-model="form.currency" class="input-premium">
                  ...
                  <template v-else>
                    <p class="text-xs font-medium text-navy-400">العملة الأساسية</p>
                    <p class="text-sm font-semibold text-navy-800">{{ currencyLabel }}</p>
                  </template>
                </div>
              </div>
            </Card>
          ...
          <!-- Submit error -->
          <div v-if="errors.submit" class="card-premium p-4 text-sm text-red-700 bg-red-50 border-red-200 mt-4">
            {{ errors.submit }}
          </div>
        </div>
        ...
        <div v-if="activeTab === 'contracts'">
          <div v-if="savedContract" class="mb-4 text-sm text-emerald-600 font-medium">تم الحفظ بنجاح!</div>

          <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <Card padding="md" title="إعدادات العقود" class="md:col-span-2">
              <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div class="space-y-1">
        ...
        <div v-if="activeTab === 'due_types'">
          <Card padding="md" title="أنواع الالتزامات">
            <!-- Inline add -->
            ...
            <div v-if="!dueTypes.length" class="text-center py-8 text-navy-400 text-sm">
              ...
              <p class="text-xs mt-1">لم يتم إضافة أي أنواع التزامات بعد.</p>
            </div>
          </Card>
        </div>
